...
| District forums + campaign meetings, listed separately | ✅ | Two tracks never merged. `CAMPAIGN.forums` = 7 district-hosted (committee outside with car painting); `CAMPAIGN.campaignMeetings` = Oct 8 Koelbel + Oct 9 Bemis, committee-hosted. Both are informational — the split is about who runs the room, confirmed with the organizer 2026-09-09. Each gets its own invite draft |
| Profile frame | ✅ | `?view=frame`. Tagline-free wordmark (the tagline is illegible clutter at avatar size); live 128/48/32px proof strip so the writer sees what survives. Four designs (Badge/Clean/Icon/Around the edge) in one picker; the badge is dragged anywhere on the photo and clamped inside the circular crop; pinch, wheel, drag, rotate, re-centre; optional citizensforlps.org curved on the ring and background soften; transparent-corner round PNG under More options. Photo picked, dropped or pasted; framed on a canvas and never uploaded. Green ring + lockup badge in 4 positions, drag/zoom, save or hand to an app via the share sheet. Ends by sending the user to write the why (Decision 060) |
...
| The picture travels to step 4 | ✅ | Review block shows the card thumbnail + "Send the picture too" toggle + link back to step 3; every panel gets phone share-sheet-with-file / desktop copy-as-image (4s timeout fallback) / save; native share sheet carries the file. Copy-as-image NOT verifiable in driven Chrome (clipboard promise hangs there); check in a normal browser |
