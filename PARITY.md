...
| Profile-photo frame (badge maker) | ✅ | `?view=frame`. Tagline-free wordmark (the tagline is illegible clutter at avatar size); live 128/48/32px proof strip so the writer sees what survives. Photo picked, dropped or pasted; framed on a canvas and never uploaded. Green ring + lockup badge in 4 positions, drag/zoom, save or hand to an app via the share sheet. Ends by sending the user to write the why (Decision 060) |
