...
The campaign palette derives from the official **Yes on 4A** logo
("INVEST FOR A STRONG LPS" — graduation cap + figure + green star,
charcoal slate lettering, green accent). Masters live in
`branding/yes-on-4a/`; every raster the site serves is GENERATED by
`tools/render_brand_assets.py` — never hand-crop a logo. The lockup
marks the site itself (header, favicon, OG card) and never a
volunteer's generated share card, which keeps the star motif
(Decision 058 amendment).
